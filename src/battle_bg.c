...
static const struct WindowTemplate sStandardBattleWindowTemplates[] = {
    ...
    [B_WIN_MOVE_TYPE] = {
        ...
        .width = 8,
        .height = 2,
        .paletteNum = B_PLTT_TYPE, // 5
        .baseBlock = B_TILE_TYPE //0x296
    },
    [B_WIN_PP_REMAINING] = {
        .bg = 0,
        .tilemapLeft = 24,
...
static void LoadBattleTerrainGfx(u16 terrain)
{
    ...
    LoadCompressedPalette(sBattleTerrainTable[terrain].palette, BG_PLTT_ID(B_PLTT_TERRAIN), 3 * PLTT_SIZE_4BPP);
}
...
{
    LoadUserWindowGfx(B_WIN_ACTION_MENU, 0x012, BG_PLTT_ID(B_PLTT_FRAME));
    LoadUserWindowGfx(B_WIN_ACTION_MENU, 0x022, BG_PLTT_ID(B_PLTT_FRAME));
    
    gPlttBufferUnfaded[BG_PLTT_ID(B_PLTT_TEXT) + 12] = RGB( 9,  9,  9);
    gPlttBufferUnfaded[BG_PLTT_ID(B_PLTT_TEXT) + 13] = RGB( 9,  9,  9);
    gPlttBufferUnfaded[BG_PLTT_ID(B_PLTT_TEXT) + 14] = RGB(31, 31, 31);
    gPlttBufferUnfaded[BG_PLTT_ID(B_PLTT_TEXT) + 15] = RGB( 26,  26,  25);
    CpuCopy16(&gPlttBufferUnfaded[BG_PLTT_ID(B_PLTT_TEXT) + 12], &gPlttBufferFaded[BG_PLTT_ID(B_PLTT_TEXT) + 12], PLTT_SIZEOF(4));

    if (gBattleTypeFlags & (BATTLE_TYPE_FIRST_BATTLE | BATTLE_TYPE_POKEDUDE))
    {
        Menu_LoadStdPalAt(BG_PLTT_ID(B_PLTT_7));
        LoadMenuMessageWindowGfx(0, 0x030, BG_PLTT_ID(B_PLTT_7));

        gPlttBufferUnfaded[BG_PLTT_ID(B_PLTT_7) + 6] = RGB( 0,  0,  0);
        CpuCopy16(&gPlttBufferUnfaded[BG_PLTT_ID(B_PLTT_7) + 6], &gPlttBufferFaded[BG_PLTT_ID(B_PLTT_7) + 6], PLTT_SIZEOF(1));
    }
    BattleUI_LoadGfx();
}
...
void LoadBattleTextboxAndBackground(void)
{
    ...
    LoadCompressedPalette(gBattleInterface_Textbox_Pal, BG_PLTT_ID(B_PLTT_UI), 2 * PLTT_SIZE_4BPP);
...
void DrawBattleEntryBackground(void)
{
    if (gBattleTypeFlags & BATTLE_TYPE_LINK)
    {
        ...
        LoadCompressedPalette(gFile_graphics_battle_transitions_vs_frame_palette, BG_PLTT_ID(B_PLTT_6), PLTT_SIZE_4BPP);
...
bool8 LoadChosenBattleElement(u8 caseId)
{
    ...
    case 5:
        battleScene = GetBattleTerrainOverride();
        LoadCompressedPalette(sBattleTerrainTable[battleScene].palette, BG_PLTT_ID(B_PLTT_TERRAIN), 3 * PLTT_SIZE_4BPP);
        break;
    case 6:
        LoadBattleMenuWindowGfx();
        break;
