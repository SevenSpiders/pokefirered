...
static const struct WindowTemplate sWindowTemplates_Moves[] = 
{
    ...
    [POKESUM_WIN_MOVES_MINI - 3] = { // Top left container for pokemon type
        .bg = 0,
        .tilemapLeft = 0,
        .tilemapTop = 3,
        .width = 0,
...
// 0 transparent, 1 white, 2 red, 3 yellow, 4 light green, 5 green, 6 light blue, 7 blue,
// 8 pink, 9 light pink, 10 light beige, 11 beige, 12 pink, 13 dark grey, 14 black
...
static void BufferMonMoveI(u8 i)
{
    ...
    {
        u8 *end;
        end = ConvertIntToDecimalStringN(sMonSummaryScreen->summary.moveAccuracyStrBufs[i], gBattleMoves[sMonSummaryScreen->moveIds[i]].accuracy, STR_CONV_MODE_RIGHT_ALIGN, 3);
        *end = CHAR_PERCENT;
        *(end + 1) = EOS;
    }
}
...
static void PokeSum_PrintTrainerMemo_Mon_HeldByOT(void)
{
    ...
    AddTextPrinterParameterized4(sMonSummaryScreen->windowIds[POKESUM_WIN_TRAINER_MEMO], FONT_NORMAL, 2, 3, 0, -1, sLevelNickTextColors[0], TEXT_SKIP_DRAW, natureMetOrHatchedAtLevelStr);
}
...
static void PokeSum_PrintAbilityNameAndDesc(void)
{
    ...
    AddTextPrinterParameterized4(sMonSummaryScreen->windowIds[POKESUM_WIN_SKILLS_ABILITY], FONT_SMALL,
                                 7, 15, 0, -1, sLevelNickTextColors[0], TEXT_SKIP_DRAW,
...
static void PokeSum_PrintMonTypeIcons(void)
{
    ...
    case PSS_PAGE_MOVES:
        break;
    case PSS_PAGE_MOVES_INFO:
        // u8 windowId = sMonSummaryScreen->windowIds[POKESUM_WIN_MOVES_MINI];
        // FillWindowPixelBuffer(windowId, 0);
        // BlitMenuInfoIcon(windowId, sMonSummaryScreen->monTypes[0] + 1, 0, 3);

        // if (sMonSummaryScreen->monTypes[0] != sMonSummaryScreen->monTypes[1])
        //     BlitMenuInfoIcon(windowId, sMonSummaryScreen->monTypes[1] + 1, 36, 3);
...
static void SwapMonMoveSlots(void)
{
    ...
    move1 = MOVE_HYPER_BEAM; //GetMonData(mon, MON_DATA_MOVE1 + sMoveSelectionCursorPos);
